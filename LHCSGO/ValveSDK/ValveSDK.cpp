...
void ValveSDK::Initialize()
{
    ...
    const auto data_cache = GetModuleHandleA("datacache.dll");
    ...
    const auto data_cache_factory = GetModuleFactory(data_cache);
    ...
    prediction = GetInterface<IPrediction> (client_factory, "VClientPrediction001");
    movement = GetInterface<IGameMovement> (client_factory, "GameMovement001");
    ...
    model_cache = GetInterface<IMDLCache>(data_cache_factory, "MDLCache004");
    ...
    move_helper = **reinterpret_cast<IMoveHelper***>(Memory::PatternScan(client, "8B 0D ? ? ? ? 8B 45 ? 51 8B D4 89 02 8B 01") + 2);
}
